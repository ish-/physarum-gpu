...
export function modifyMatShader (mat, {
  ...
  mat.onBeforeCompile = shader => {
    ...
    if (vertex) {
      shader.vertexShader = vertex.startsWith(mmsSym) ?
        modifyShader(shader.vertexShader, vertex)
      : vertex;
    }
    if (fragment) {
      shader.fragmentShader = fragment.startsWith(mmsSym) ?
...
export function modifyShader (shader, payload) {
  ...
  const chunks = payload.split(mmsSym);
...
export function guiCtrlWidth (w, ctrl) {
