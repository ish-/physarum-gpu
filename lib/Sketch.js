...
import { gui, ctrlWidth } from '/lib/gui';
import { onTouchStart, onTouchEnd, onTouchMove } from '/lib/utils';

class Sketch extends EventDispatcher {
  scene = new Scene();
  exts = {};
  size = new Vector2(window.innerWidth, window.innerHeight);
  timeUniform = {x: 0, y: 0, z: 0, w: 0};
  ...
  __raf (onRender) { // requestAnimationFrame
    ...
    this.timeUniform.x = elapsed;
    this.timeUniform.y = delta;
    this.timeUniform.z = now;
  ...
  hidePanels (bool) {
    ...
    gui.domElement.style.display = this._hidden ? 'none' : '';
  }
  ...
  initGui () {
    ctrlWidth(50, gui.add(this, 'setFullscreen').name('fullscreen (F)'));
    ctrlWidth(50, gui.add(this, 'hidePanels').name('hide panels (H)'));
