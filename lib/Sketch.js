...
class Sketch extends EventDispatcher {
  ...
  initPointer ({ left = 1, right = -1} = {}) {
    ...
    onTouchStart($canvas, e => handleTouch(e, touchDown = left));
    onTouchEnd($canvas, e => handleTouch(e, touchDown = 0));
    onTouchMove($canvas, handleTouch);
    $canvas.addEventListener('contextmenu', e => {
      e.preventDefault();
      handleTouch(e, touchDown = right)
    });
