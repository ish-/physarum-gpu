...
export default class Feedback {
  ...
  constructor({
    ...
    this.isSketchSize = !width && !!renderer;
    if (this.isSketchSize) {
      const size = new Vector2(); renderer.getSize(size);
      width = width || size.x || window.innerWidth;
      height = height || size.y || window.innerHeight;

      sketch.addEventListener('resize', ({ size: { x, y } }) => this.setSize(x, y))
    }
    console.log(`new ${ name }`, this);
    ...
    if (compute) {
      compute = compute.replace('vec4 compute', 'vec4 _compute');
  ...
  initWithTexture (texture = this.initTexture) {
    ...
    this.compFsMaterial.uniforms.tInit.value = texture;
  }

  setSize (w, h) {
    console.log('Feedback.setSize()', w, h);
